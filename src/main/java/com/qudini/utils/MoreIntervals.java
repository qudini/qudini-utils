...
import com.qudini.utils.intervals.Interval;
import com.qudini.utils.intervals.UpdatableInterval;
...
public final class MoreIntervals {
    ...
    public static <E extends Comparable<? super E>, T extends UpdatableInterval<E, T>> List<T> enclose(List<T> intervals, Interval<E, ?> bounds) {
        return intervals
                .stream()
                .filter(interval -> MoreComparables.isGreaterThan(interval.getEnd(), bounds.getStart()))
                .filter(interval -> MoreComparables.isLessThan(interval.getStart(), bounds.getEnd()))
                .map(interval -> interval.withStart(MoreComparables.greatestBetween(interval.getStart(), bounds.getStart())))
                .map(interval -> interval.withEnd(MoreComparables.leastBetween(interval.getEnd(), bounds.getEnd())))
                .toList();
    }
    ...
    public static <E extends Comparable<? super E>, T extends UpdatableInterval<E, T>> List<T> merge(List<T> intervals) {
        ...
        for (int i = 1, n = intervals.size(); i < n; i++) {
            ...
            if (a.overlaps(b) || a.isContiguousWith(b)) {
                a = a.withStart(MoreComparables.leastBetween(a.getStart(), b.getStart()));
                a = a.withEnd(MoreComparables.greatestBetween(a.getEnd(), b.getEnd()));
            } else {
    ...
    private static <E extends Comparable<? super E>, T extends UpdatableInterval<E, T>> Stream<T> subtract(T interval, Interval<E, ?> subtrahend) {
        ...
        } else if (MoreComparables.isLessThanOrEqualTo(interval.getStart(), subtrahend.getStart())) {
